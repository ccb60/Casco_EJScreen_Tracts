...
rename(`Life Expectancy` = p_NEG_LIFEEXP,
       `Income` = p_LOWINCPCT, 
       `Education` = p_LESSHSPCT, 
       `Language` = p_LINGISOPCT, 
...
ranks, and dividing my the overal lsample size.
...
tmp <- cb_data %>% 
  ...
  mutate(across(LOWINCPCT:NEG_LIFEEXP, function(x) 
                                       rank(x, na.last = 'keep', 
                                            ties.method = 'average')/81)) %>%
rename(`Life Expectancy` = NEG_LIFEEXP,
       `Income` = LOWINCPCT, 
       `Education` = LESSHSPCT, 
       `Language` = LINGISOPCT, 
...
cb_geospatial_df_2 <- cb_geospatial_df %>%
  pivot_longer(`Income`:`Life Expectancy`,
...
plt <- ggplot() +
  ...
  scale_fill_viridis_b(option = 'B', na.value = "grey75", name = 'Rank') +
  theme_void() +
  theme(panel.background = element_rect(fill = 'grey85'),
        legend.position = 'bottom') +
  coord_equal(xlim = xlims, ylim = ylims)
