...
#library(rgeos)  # Needed for polygon calcs without problematic dependency
#library(broom)  # used to tidy geospatial info to dataframe for ggplot
                 # but can use could use fortify() from ggplot as well.)
```
...
```{r}
plt <- ggplot() +
  geom_sf(data = cb_geospatial, 
               fill="#69b3a2", color="white") +
  geom_sf(data = cb_watershed_outline,
            color="grey40") +
...
plt <- ggplot() +
  geom_sf(data = Maine,  
                fill = 'grey50',
               color='grey10')  +
  geom_sf(data = cb_geospatial_2, aes(fill = Index_2), color="white") +
  geom_sf(data = cb_watershed_outline,  
            color="grey40") +
  geom_sf(data = Maine,  fill = NA,
                color='grey10')  +
...
```{r}
ggsave('figures/vulnerability_index.png', type='cairo',
         width = 3, height = 3)
ggsave('figures/vulnerability_index.pdf', device = cairo_pdf, 
       width = 3, height = 3)
```
...
rename(`Low Life Expectancy` = P_NEG_LIFEEXP,
...
plt <- ggplot() +
  ...
  geom_sf(data = cb_geospatial_3, aes(fill = Value), color="grey60") +
...
tmp <- cb_data %>% 
  ...
  mutate(across(LOWINCPCT:NEG_LIFEEXP, function(x) 
                                       rank(x, na.last = 'keep', 
                                            ties.method = 'average')/81)*100) %>%
...
cb_geospatial_5 <- cb_geospatial_4 %>%
  pivot_longer(`Low Income`:`Low Life Expectancy`,
...
plt <- ggplot() +
  geom_sf(data = Maine,  
                fill = 'grey50', color=NA)  +
  geom_sf(data = cb_geospatial_5, aes(fill = Value), color='grey60') +
  geom_sf(data = cb_watershed,  color="white", fill = NA) +
  
  facet_wrap(~ Indicator, nrow = 2) +
  
  scale_fill_viridis_b(option = 'B',  name = 'Local/nPercentile',
                       na.value = "grey75", breaks = c(0, 20, 40, 60, 80, 100 )) +
